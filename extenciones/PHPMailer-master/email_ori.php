...
foreach ($respuestaVenta as $key => $venta) {
	...
	try {
	    ...
	    $mail->Host       = 'mail.systsolutions-ec.com';                    // Set the SMTP server to send through
	    $mail->SMTPAuth   = true;                                   // Enable SMTP authentication
	    $mail->Username   = 'notificaciones@systsolutions-ec.com';                     // SMTP username
	    $mail->Password   = 'Systsolutions-EC*2020*';                               // SMTP password
	    $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` also accepted
	    $mail->Port       = 587;                                    // TCP port to connect to
	    //Recipients
	    $mail->setFrom('notificaciones@systsolutions-ec.com', 'LA TERNERA');
...
foreach ($retenciones as $key => $retencion) {
	...
	try {
	    ...
	    $mail->SMTPAuth   = true;                                   // Enable SMTP authentication
	    $mail->Username   = 'notificaciones@systsolutions-ec.com';                     // SMTP username
	    $mail->Password   = 'Syst2020*';                               // SMTP password
	    $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` also accepted
	    $mail->Port       = 587;                                    // TCP port to connect to
	    //Recipients
	    $mail->setFrom('notificaciones@systsolutions-ec.com', 'LA TERNERA');
